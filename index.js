...
const createArrowBody = (context) => {
  ...
  const lintVoid = (node, scope) => {
    ...
    const hasComments =
      context.getSourceCode().getCommentsInside(block).length > 0;
    context.report({
      node: block,
  ...
  const lintBody = (node, scope) => {
    ...
    if (
      !expressions.every((expression) =>
        SIMPLE_SIDE_EFFECT_EXPRESSIONS.includes(expression.type)
      )
    )
      return;
    const fix = (fixer) => {
    ...
    const hasComments =
      context.getSourceCode().getCommentsInside(voidExpression).length > 0;
    context.report({
      node: voidExpression,
  ...
  rules: {
    ...
    "arrow-body": {
      ...
      meta: {
        ...
        docs: {
          description:
            "Prefer void operator or function body to void implicit arrow function return values.",
          suggestion: true,
        },
        fixable: "code",
        schema: [
